...
class DefaultController extends Controller
{
    ...
    public function ezTextingIndexAction(Request $request){
            ...
            if ( 'Failure' == $json->Status ) {
                ...
                foreach ( $json->Entries as $message ) {
                    if($message->FolderID == 0){
                        ...
                        $contact = $contactController->getOneContact_Normal($contactID);
                        ...
                        if ($contact['data'] && sizeof($contact['groups']) > 0){
                            ...
                            $note= $contact['note'];
                            $array= array('None');
                            $contactController->updateContact_Normal($id, $phoneNumber, $firstName, $lastName, $email, $note,$array);
                        }
                    }
                }
        ...
        $managingInboxController->moveMessageToFolder_Normal($messageIds, Configuration::moveFolderId);
